...
def differential_cryptoanalysis(
    enc_key, size, deltaP, expected_deltaU4, key_mask, prob
):
    ...
    hit_table = [0] * (2 ** (16))
    max_hit_table = [
        {"key": 0, "prob": 0, "Q": 1} for i in range(HIT_TABLE_SIZE)
    ]  # key, prob, Q
...
def main():
    # %%%%%% DEFAULT ARGS %%%%%% #
    enc_key = 0x1345  # normally, this is unknown, and not used later
    ...
    # Overwrite args
    enc_key = 0x1345  # key for encryption
    size = 100  # number of plaintext block generated
    deltaP = 0b1011_0000_1011_0000  # what bits differ at P
    expected_deltaU4 = 0b0000_1000_0000_1000  # what bits must differ at U4
    key_mask = 0b0101  # Sbox mask
    prob = 9 / 256

    max_hits = differential_cryptoanalysis(
        enc_key, size, deltaP, expected_deltaU4, key_mask, prob
    )

    max_hits = sorted(max_hits, key=lambda x: x["prob"], reverse=True)

    print(f"Top {HIT_TABLE_SIZE} observed probabilities:")
    for res in max_hits:
