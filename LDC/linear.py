...
def linear_cryptoanalysis(pt_file, ct_file, pmask, umask, key_mask, bias):
    ...
    hit_table = [0] * (2 ** (16))
    max_hit_table = [
        {"key": 0, "bias": 0, "Q": 1} for i in range(HIT_TABLE_SIZE)
    ]  # key, bias, Q
    ...
    with open(pt_file, "r") as fid_pt, open(ct_file, "r") as fid_ct:
        ...
        for key in possible_keys:
            ...
            if Q < max_hit_table[-1]["Q"]:
                ...
                max_hit_table[-1]["Q"] = Q
                max_hit_table = sorted(
                    max_hit_table, key=lambda x: x["Q"], reverse=False
                )
...
def main():
    ...
    # Override args
    P_mask = 0b1010_0000_1010_0000  # Plaintext bitmask
    U_mask = 0b0000_0100_0000_0100  # U mask
    key_mask = 0b0101  # Sbox mask
    bias = 1 / 32

    max_hits = linear_cryptoanalysis(
        args["plaintext"], args["ciphertext"], P_mask, U_mask, key_mask, bias
    )

    max_hits = sorted(max_hits, key=lambda x: x["bias"], reverse=True)

    print(f"Top {HIT_TABLE_SIZE} biases:")
    for i, res in enumerate(max_hits):
        print(
            f"{i}. key: 0x{res['key']:04X}, bias: {res['bias']:.4f}, Q: {res['Q']:.4f}"
        )
